...
export const renderImage = ({getImgPath, getExtraKey, getDataKey, item}) => {
    let img
    let checkImg
    let url_plchdlr = item['pmWorkOrder']
    let placeholder = !url_plchdlr?PLACEHOLDER_URL:PLACEHOLDER_URL_PMs
    let assetImage = item?item[getDataKey]:null

    if(assetImage!==null||undefined) {
        checkImg = !getExtraKey?(item[getDataKey]!==undefined||null?item[getDataKey]:[]):[]
